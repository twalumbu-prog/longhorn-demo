    ...
    <div ref={container} className="min-h-screen bg-brand-soft pt-44 pb-32 px-6 overflow-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="transition-card mb-24 relative p-12 md:p-20 bg-white rounded-[4rem] shadow-[0_20px_50px_rgba(0,0,0,0.03)] border border-gray-100 max-w-6xl mx-auto overflow-hidden group">
          {/* Decorative Elements */}
          <div className="absolute top-0 right-0 w-96 h-96 bg-brand-red/5 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/2 group-hover:bg-brand-red/10 transition-colors duration-700"></div>
          <div className="absolute bottom-0 left-0 w-64 h-64 bg-brand-red/5 rounded-full blur-[80px] translate-y-1/2 -translate-x-1/4"></div>
          ...
          <div className="relative z-10 text-center">
            ...
            <h2 className="font-heading text-5xl md:text-7xl font-bold text-gray-900 mb-8 leading-[1.1]">
              Architecting Your <span className="italic text-brand-red">Digital Longhorn</span>
            </h2>
            ...
            <div className="flex flex-col sm:flex-row gap-5 justify-center items-center">
              <button 
                onClick={openModal}
                className="bg-brand-red text-white px-12 py-5 rounded-full font-bold uppercase tracking-widest text-xs hover:bg-brand-red/90 transition-all shadow-xl hover:-translate-y-1 active:translate-y-0 min-w-[240px]"
              >
                Request Full Product
              </button>
              <button 
                onClick={openModal}
                className="bg-white border border-gray-200 text-gray-900 px-12 py-5 rounded-full font-bold uppercase tracking-widest text-xs hover:border-brand-red transition-all hover:-translate-y-1 active:translate-y-0 min-w-[240px]"
              >
                Consult Blue Opus
              </button>
            </div>
            ...
            <div className="mt-16 pt-12 border-t border-gray-100 max-w-2xl mx-auto text-center">
              ...
              <div className="flex items-center justify-center gap-3">
                 ...
                 <span className="font-heading text-2xl font-bold text-brand-red tracking-wider">BLUE OPUS</span>
                 <div className="w-8 h-px bg-gray-200"></div>
              </div>
              <p className="text-sm text-gray-500 mt-4 leading-relaxed max-w-md mx-auto italic font-medium">
                "The development and provision of this world-class website design is exclusively handled by the <span className="text-brand-red">Blue Opus</span> digital crafting team."
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="pricing-header text-center mb-20">
          <h3 className="text-brand-red font-bold uppercase tracking-[0.3em] text-xs mb-8">Strategic Investment</h3>
          ...
          {packages.map((pkg, i) => (
            <div 
              ...
              className={`pricing-card relative flex flex-col p-10 md:p-12 rounded-[3.5rem] transition-all duration-700 hover:-translate-y-3 shadow-sm hover:shadow-2xl ${
                ...
                pkg.isPopular ? 'scale-[1.02] lg:scale-105 z-10 ring-1 ring-brand-red/10' : ''
              }`}
            >
              {/* Most Popular Badge */}
              {pkg.isPopular && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-brand-red text-white px-6 py-2 rounded-full text-[10px] font-bold uppercase tracking-[0.2em] flex items-center gap-2 shadow-lg">
                  <Star size={12} className="fill-brand-red text-brand-red" /> Most Popular
                </div>
              )}
              ...
              <div className="mb-10">
                ...
                <div className="flex items-baseline gap-2 mb-4">
                  ...
                  <span className={`text-5xl font-black tracking-tighter ${pkg.isElite ? 'text-brand-red' : 'text-gray-900'}`}>{pkg.price}</span>
                </div>
                ...
                {pkg.features.map((feat, j) => (
                  <div key={j} className="flex gap-4 items-start group/feat">
                    <div className={`mt-0.5 w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 transition-colors ${
                      pkg.isElite ? 'bg-white/10 group-hover/feat:bg-brand-red/20' : 'bg-brand-soft group-hover/feat:bg-brand-red/10'
                    }`}>
                      <Check size={12} className={pkg.isElite ? 'text-brand-red' : 'text-brand-red'} />
                    </div>
              ...
              <button 
                ...
                className={`w-full py-5 rounded-[2rem] font-bold uppercase tracking-widest text-[10px] transition-all flex items-center justify-center gap-3 overflow-hidden group/btn ${
                  pkg.isElite 
                    ? 'bg-white text-gray-950 hover:bg-brand-red hover:text-white' 
                    : pkg.isPopular 
                      ? 'bg-brand-red text-white hover:bg-gray-900' 
                      : 'bg-white border-2 border-gray-100 text-gray-900 hover:border-brand-red hover:text-brand-red shadow-sm'
                }`}
              >
        ...
        <div className="max-w-4xl mx-auto flex flex-wrap justify-center gap-12 text-center text-gray-400">
           <div className="flex flex-col items-center">
             <CheckCircle2 size={24} className="mb-3 text-brand-red opacity-50" />
             <span className="text-[10px] font-bold uppercase tracking-widest">Post-Launch Support</span>
           </div>
           <div className="flex flex-col items-center">
             <TrendingUp size={24} className="mb-3 text-brand-red opacity-50" />
             <span className="text-[10px] font-bold uppercase tracking-widest">Growth Focused</span>
           </div>
           <div className="flex flex-col items-center">
             <ShieldCheck size={24} className="mb-3 text-brand-red opacity-50" />
